...
exports.getMostBookedHotel = async ({ month, roomRating = 0 }) => {
  ...
  const suitedBookedRooms = await sequelize.query(
    'SELECT bks.id, bks.move_in_date, bks.leave_date, bks.is_cancelled, rooms.id AS room_id, rooms.img, rooms.rating, rooms.cost, rooms.hotel_id FROM booked_rooms AS bks INNER JOIN rooms ON month(bks.created_at) = :month AND rooms.rating > :rating AND bks.room_id = rooms.id;',
    {
